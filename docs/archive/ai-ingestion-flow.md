...
## L1–L4 Cache Tier Hierarchy

How each cache tier is consulted on a brief request. L2 is checked first (inputs assembled before the L1 gate); L3 and L4 are only reached on an L1 miss.

```mermaid
flowchart TD
    REQ["GET /api/customer/:name/brief"] --> FETCH

    subgraph FETCH ["① Input Assembly — always runs, parallel"]
        direction LR
        L2A["L2 — Email cache\n2h TTL"]
        L2B["L2 — Meeting cache\n2h TTL"]
        L3A["L3 — CCSP data\nhash-guarded disk cache"]
        L3B["L3 — Pipeline data\nhash-guarded disk cache"]
    end

    FETCH --> FP["Compute fingerprint over all inputs"] --> L1{"L1 — Brief cache\nFingerprint match?"}

    L1 -->|"✅ HIT — serve immediately"| RETURN["Cached brief returned\n(L3 docs, L4 not consulted)"]
    L1 -->|"❌ MISS — inputs changed"| L3DOCS

    subgraph L3DOCS ["③ Document Intelligence — on L1 miss only"]
        direction LR
        DC["L3 — Doc content\nfileId+modTime\n(content-addressed)"]
        DCC["L3 — Doc classification\nfileId+modTime\n(content-addressed)"]
    end

    L3DOCS --> L4

    subgraph L4 ["④ Account + Industry Intelligence — on L1 miss only"]
        direction LR
        AI["L4 — Account intel\n14d TTL + contentHash"]
        IND["L4 — Industry analysis\n30d TTL\n(shared by industry+region)"]
    end

    L4 --> GEN["Generate brief\n(delta or full-run)"]
    GEN --> WRITE["Write to L1\n(text + fingerprint + corpusSnapshot)"]
    WRITE --> RESP["Return generated brief"]

    style L1 fill:#27ae60,color:#fff
    style RETURN fill:#2980b9,color:#fff
    style GEN fill:#16a085,color:#fff
