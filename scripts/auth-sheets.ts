/**
 * One-time auth flow for Google Sheets.
 * Run: bun scripts/auth-sheets.ts
 * Opens a browser URL, catches the redirect, saves the token.
 *
 * Requires Google Sheets API to be enabled in your GCP project:
...
console.log('\n📊  Google Sheets — one-time auth\n')
console.log('Prerequisite: enable Sheets API in GCP console if not already done.')
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    res.end('<h2 style="font-family:sans-serif">✅ Sheets authorized — you can close this tab.</h2>')
