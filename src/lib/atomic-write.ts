 ...
 *
 * Three operations:
 *   - writeJsonAtomic(path, value, opts?)  — JSON.stringify + atomic rename, with stale-overwrite guard
 *   - writeFileAtomic(path, data, opts?)   — raw bytes/string + atomic rename
 *   - appendLine(path, line, opts?)        — append-only log line (NOT atomic; ordering > crash safety)
 *
 * Stale-overwrite guard (writeJsonAtomic only):
