...
export function sanitizeComposedEmail(text: string, recipientName: string): string {
  let result = text

  const replacements: Array<[RegExp, string]> = [
    [/\bleverage Red Hat\b/gi, 'use Red Hat'],
    [/\bleverage our\b/gi, 'use our'],
    [/\bleverage a Red Hat\b/gi, 'use a Red Hat'],
    [/\bleverage\b/gi, 'use'],
    [/\bhighlight\b/gi, 'note'],
    [/\bdemonstrate\b/gi, 'show'],
    [/\bemphasize\b/gi, 'focus on'],
    [/\bposition\b(?=\s+(?:Red Hat|RHEL|OpenShift|Ansible|AAP|the platform))/gi, 'present'],
  ]

  for (const [pattern, replacement] of replacements) {
    result = result.replace(pattern, (match) => {
      const isCapitalized = match[0] === match[0].toUpperCase() && match[0] !== match[0].toLowerCase()
      const final = isCapitalized ? replacement.charAt(0).toUpperCase() + replacement.slice(1) : replacement
      console.log(`[sanitize] ${recipientName}: replaced "${match}" → "${final}"`)
      return final
    })
  }

  result = result.replace(/[^.!?\n]*\b(?:support\s+case|support\s+ticket)\b[^.!?\n]*[.!?]?\s*/gi, (match) => {
    console.log(`[sanitize] ${recipientName}: removed sentence "${match.trim()}"`)
    return ''
  })
...
<h2 style="font-size: 14px; text-transform: uppercase; letter-spacing: 2px; color: ${BRAND_RED}; margin: 0 0 8px 0;">📧 Executive Outreach</h2>
...
${execEmailsHtml.length > 0 ? execEmailsHtml.join('\n') : '<p style="font-size: 14px; color: #9aa0a6; font-style: italic; margin: 0 0 20px 0;">No executive contacts identified</p>'}

<h2 style="font-size: 14px; text-transform: uppercase; letter-spacing: 2px; color: ${BRAND_RED}; margin: 24px 0 8px 0;">📧 Manager Outreach</h2>
...
${managerEmailsHtml.length > 0 ? managerEmailsHtml.join('\n') : '<p style="font-size: 14px; color: #9aa0a6; font-style: italic; margin: 0 0 20px 0;">No manager contacts identified</p>'}
