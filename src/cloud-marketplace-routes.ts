  ...
  app.get('/api/customer/:name/cloud-marketplace', async (c) => {
    ...
    try {
      ...
      const providers = summarySignals.map(s => ({
        ...
        offerings: (s.metadata?.offerings ?? []).map((o: any) => ({ ...o, sourceUrl: o.sourceFileId ? `https://docs.google.com/presentation/d/${o.sourceFileId}` : undefined })),
        programs: (s.metadata?.programs ?? []).map((p: any) => ({ ...p, sourceUrl: p.sourceFileId ? `https://docs.google.com/presentation/d/${p.sourceFileId}` : undefined })),
        incentives: (s.metadata?.incentives ?? []).map((i: any) => ({ ...i, sourceUrl: i.sourceFileId ? `https://docs.google.com/presentation/d/${i.sourceFileId}` : undefined })),
