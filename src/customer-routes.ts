...
export function registerCustomerRoutes(app: Hono): void {
  ...
  app.post('/api/briefs/pregen-all', (c) => {
    ...
    ;(async () => {
      console.log(`[briefs/pregen-all] starting background brief pre-generation for ${missing.length} customers`)
    ...
    return c.json({ message: 'Brief pre-generation started', queued: missing.length })
  })
