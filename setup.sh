...
PORT="${PORT:-7777}"
CONTAINER_NAME="${CONTAINER_NAME:-pai-dashboard}"
DASHBOARD_URL="http://localhost:${PORT}/dashboard/setup"
HEALTH_URL="http://localhost:${PORT}/api/aes"
...
start_container() {
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    say "(dry-run) would run: podman run -d --name $CONTAINER_NAME -p ${PORT}:7777 $IMAGE_REF"
    return 0
  fi
  ...
  # Build env-file arg only if .env exists (challenge mode skips it)
  local env_file_arg=()
  if [[ -f .env ]]; then
