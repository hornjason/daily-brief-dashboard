...
**Product scrape completeness contract (#829):** Completeness means page-visible items match captured items — NOT `_product.json` items match downloads. The source of truth is the PRODUCT PAGE, not `_product.json`. After every scrape: (1) capture a full-page text dump of visible content, (2) extract every document name visible on the page, (3) compare against `_product.json` items, (4) flag MISSING items — documents visible on the page that aren't in `_product.json`. A manifest showing "0 gaps" when DocListPicker sections have uncaptured items is WRONG — the gap is between the page and the JSON, not between the JSON and downloads. Never report completeness without comparing against page content. The Seismic API does NOT return DocListPicker items — this is a KNOWN gap that must be explicitly listed in every completeness manifest, not hidden behind "0 gaps."
