...
### BKL-HERO-PARENT-REGION-BUG — Bootstrap creates folders at Drive root when adding AEs from non-default region
- Status: ✅ DONE 2026-05-08
- Priority: P1
- Issue: #74
...
- Decision: DONE — Three-part fix: (1) Added OAuth keys prerequisite check to startup product refresh (background-scheduler.ts lines 1292-1300), matching pattern from territory sync line 581. If keys missing, logs skip message and doesn't run refresh. (2) Added POST /api/products/refresh-all endpoint (product-intel-routes.ts lines 687-702) to manually trigger refresh of all products. (3) Bootstrap completion now triggers product refresh via new endpoint in both POD bootstrap (lines 835-838) AND auto-bootstrap (lines 1195-1198 + baseUrl at 886) paths after OAuth keys are created, so fresh installs get products immediately after wizard completes instead of waiting until Sunday. Image 68f35baa3bcd published to ghcr.io as :latest and :stable. Commits: 3459f46 (initial), 3f38e50 (auto-bootstrap trigger), 93b4da4 (baseUrl scope fix). **Lesson:** Should have identified both bootstrap paths, verified all variable scopes, AND tested the implementation before committing — incomplete design understanding plus untested code caused three-pass fix.
...
BKL-CAMP-01 → #439 — Campaign templateAll() migration: campaign-service now calls templateAll (DONE 2026-05-27)
BKL-ARCH-COMPLIANCE-01 → #441 — Architecture compliance test catches templateAll bypass (DONE 2026-05-27)
BKL-PLAYBOOK-EXPORT-01 → #440 — Playbook export parity verified: playbook-to-markdown.ts has all sections (DONE 2026-05-27)
BKL-SALESHUB-TDP-01 → #436 — SalesHub TDP extraction: services, whatToShare, extractedContent fixed (DONE 2026-05-27)
BKL-SALESHUB-L3-01 → #442 — SalesHub L3 sync path: hero container reads Drive JSON (DONE 2026-05-27)
BKL-MTGPREP-PIPELINE-01 → #444 — Meeting prep shows opp name + amount instead of bare IDs (DONE 2026-05-27)
BKL-MTGPREP-LINKBACK-01 → #445 — Meeting prep surfaces SalesHub assets with linkbacks (DONE 2026-05-27)
