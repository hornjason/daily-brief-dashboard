...
| `/dashboard/customer/:name` | CustomerDetailPage | ✅ Working | v2.0 redesign: slim header (health popover, Meeting Prep), left column (Brief → Sales Strategy → AI Opportunities → What Changed → Deep Dive), 3 sidebar groups (Opportunities, People, Account Data), floating Customer Intelligence Q&A widget (queries all customer data via Gemini, no web grounding) |
...
### Feature Modules
- `GET /api/modules/status` — Registry status for all modules (31 modules, auto-discovered; includes territory-module, meeting-context)
