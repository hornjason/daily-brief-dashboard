...
const { callGeminiStructured } = await import('../../src/doc-extraction.ts')
...
// ── BKL-TEST-P0-04b: 429 retry extended to brief pipeline + doc-classification ──
//
// Why these tests exist:
//   callGeminiGrounded in account-intelligence.ts had the 429-retry contract
//   covered by the tests above. The rest of the pipeline did NOT — callLLM and
//   callLLMStructured (customer.ts, used for brief-synthesize and brief-extract)
//   and callGeminiStructured (doc-extraction.ts, used for doc-classify) would
//   throw on the first 429 with no retry. Under sustained Vertex quota pressure
//   that meant partial-failure briefs — some customers got full briefs, others
//   got hard errors mid-pipeline.
//
//   BKL-TEST-P0-04b extends the same retry-with-backoff pattern (4 retries,
//   exponential 2s/4s/8s/16s + jitter, throw "Gemini 429 after 4 retries" on
//   exhaustion) to those three call sites. These tests guard the contract.
//
// Mock strategy mirrors the block above exactly:
//   - setTimeout spy with immediate-invoke so the 4 retry delays (~30s) don't
//     block CI.
//   - fetch spy returning 429 on every call to drive retry to exhaustion.
//   - ./google.ts and ./gemini-cost-tracker.ts mocked at module-load time
//     (top of this file) so the SUT fallback OAuth path works without disk.

describe('BKL-TEST-P0-04b: brief pipeline (callLLM / callLLMStructured) 429 retry', () => {
  let fetchSpy: ReturnType<typeof spyOn>
  let setTimeoutSpy: ReturnType<typeof spyOn>
  ...
  test('callLLM (brief-synthesize) retries on 429 and throws on exhaustion', async () => {
    let captured: unknown = null
    try {
      const result = await callLLM('system prompt', 'user prompt', 'brief-synthesize', 'TestCustomer')
      expect.unreachable(
        `callLLM returned on 429 instead of throwing. ` +
          `Returned: ${JSON.stringify(result).slice(0, 200)}`,
      )
    } catch (e) {
      captured = e
    ...
    expect(captured).toBeInstanceOf(Error)
    const msg = (captured as Error).message
    // Exhaustion path throws "Gemini 429 after 4 retries — rate limited".
    // Non-429 retry throws "Gemini API error NNN: ...". Both name 429/rate-limit.
    expect(msg).toMatch(/429|rate.?limit/i)
    // Retry loop must have run — initial call + ≥1 retry.
    expect(fetchSpy.mock.calls.length).toBeGreaterThanOrEqual(2)
    // Backoff sleep must have happened.
    expect(setTimeoutSpy.mock.calls.length).toBeGreaterThanOrEqual(1)
    // Anti-thin-doc: error must not start with raw JSON body.
    expect(msg.startsWith('{')).toBe(false)
  })

  test('callLLMStructured (brief-extract) retries on 429 and throws on exhaustion', async () => {
    const schema = {
    ...
    let captured: unknown = null
    try {
      const result = await callLLMStructured('system', 'user', schema, 'brief-extract', 'TestCustomer')
      expect.unreachable(
        `callLLMStructured returned on 429 instead of throwing. ` +
          `Returned: ${JSON.stringify(result).slice(0, 200)}`,
      )
    } catch (e) {
      captured = e
...
describe('BKL-TEST-P0-04b: doc-classification (callGeminiStructured) 429 retry', () => {
  let fetchSpy: ReturnType<typeof spyOn>
  let setTimeoutSpy: ReturnType<typeof spyOn>
  ...
  test('callGeminiStructured (doc-classify) retries on 429 and throws on exhaustion', async () => {
    const schema = {
      type: 'object' as const,
    ...
    let captured: unknown = null
    try {
      const result = await callGeminiStructured('system', 'user', schema)
      expect.unreachable(
        `callGeminiStructured returned on 429 instead of throwing. ` +
          `Returned: ${JSON.stringify(result).slice(0, 200)}`,
      )
    } catch (e) {
      captured = e
